...
public:
    ...
    */
    enum concurrency_strategy { primitiveX, primitiveY, primitiveZ, subDomain, serial };
    ...
    struct SubDomain {
         ...
         */
        void addIndex(const std::array<int, 3> &indexArray, int index, CellContainerIterators &cell);
     ...
     */
    enum boundary_conditions { outflow, periodic, reflective };
     ...
     * @param concurrency_strategy the multi threading strategy that should be used
     */
    CellContainer(double Xv,
                        double Yv,
                        double Zv,
                        double rCutV,
                        std::array<boundary_conditions, 6> borderV = std::array<boundary_conditions, 6>{
                                outflow, outflow, outflow, outflow, outflow, outflow},
                        std::array<double, 3> g = std::array<double, 3>{0., 0., 0.},
                        concurrency_strategy strategy=serial);
     ...
     * @return iterator
     */
    std::vector<CellContainerIterators>::iterator begin_cell();

    /**
     * @brief Provides the iterator for single particles at the end of the collection
     * @return iterator
     */
    std::vector<CellContainerIterators>::iterator end_cell();

    /**
     * @brief Provides the _const_ iterator for single Cells at the start of the collection
     * @return const iterator
     */
    [[nodiscard]] std::vector<CellContainerIterators>::const_iterator begin_cell() const;

    /**
     * @brief Provides the _const_ iterator for single Cells at the end of the collection
     * @return const iterator
     */
    [[nodiscard]] std::vector<CellContainerIterators>::const_iterator end_cell() const;
     ...
     * @return
     */
    [[nodiscard]] std::tuple<boundary_conditions, int> getBorders(const std::array<int, 3> &currentIndexes, int d) const;
    ...
    [[nodiscard]] const std::vector<CellContainerIterators> &getGrid() const;
    ...
    /***** Setters *****/
    void setGrid(const std::vector<CellContainerIterators> &grid);
    ...
    [[nodiscard]] const std::array<boundary_conditions, 6> &getBorder() const;

    void setBorder(const std::array<boundary_conditions, 6> &border);
    ...
    [[nodiscard]] concurrency_strategy getStrategy() const;

    void setStrategy(concurrency_strategy strategy);
     ...
     */
    std::vector<CellContainerIterators> grid;
    /**
...
private:
     ...
     */
    std::array<boundary_conditions, 6> border{outflow};
...
double CellContainer::getDistance(const std::array<double, 3> &X, int bord) const {
    switch (bord) {
        // LEFT
        case 0: return X[0];
            // RIGHT
...
bool inline CellContainer::isNeighborInRange(const Particle *p, const std::array<int, 3> &neighbor) {
...
int CellContainer::index(const std::array<int, 3> &currentIndexes) const {
