...
- The observed increase in cache misses can have a substantial influence on the performance, because accessing cache data is much faster than retrieving information from the RAM. This rise in cache misses might be due to false sharing between the threads, meaning the threads invalidate each others local caches due to changing data in the same cache line. From the description above it follows, that also threads access arbitrary points in our vector of particle objects. Therefore the different threads might regularly access the same cache lines, which then leads to false sharing 
